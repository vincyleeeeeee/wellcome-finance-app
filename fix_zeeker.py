...
sb.table('projects').update({'currency':'RMB'}).eq('project_code','WELL202608ZK003').execute()
r=sb.table('projects').select('*').eq('project_code','WELL202608ZK003').execute()
p=r.data[0]
c=sb.table('clients').select('*').eq('id',p['client_id']).execute(); client=c.data[0]
proj={'client_short':client['short_name'],'project_code':p['project_code'],
      'project_name':p.get('project_name',''),'brand_name':p.get('brand_name',''),
      'amount':41310.53,'currency':'RMB','venue':'吉隆坡',
      'execution_period':p.get('execution_period',''),'shooting_date':p.get('shooting_date',''),
      'total_posts':p.get('total_posts',''),'invoice_date':__import__('datetime').datetime.now(),
      'due_date':p.get('due_date'),'invoice_project_name':p.get('brand_name','')}
...
out='/Users/vincy/Documents/Wellcome/项目/Zeekr/ZEEKER/财务/Zeeker-invoice-stamped.pdf'
