...
def generate_invoice(client: dict, project: dict) -> str:
    ...
    ws['E15'] = 1
    ws['G15'] = amount
    _fmt = '¥#,##0.00' if is_rmb else '$#,##0.00'
    for _c in ('D15', 'G15', 'G17', 'G18', 'G19'):
        ws[_c].number_format = _fmt
    ...
    if client.get('full_name','').upper().startswith('INFINIX'):
        swift_cell = ws['C31']
        if swift_cell.value and 'UBHKHKHH' in str(swift_cell.value):
            swift_cell.value = str(swift_cell.value).replace('UBHKHKHH', 'UBHKHKHHXXX')
