...
div[data-testid="stCheckbox"] label { font-size: 17px !important; }
input, select, textarea, .stTextInput input, .stNumberInput input { font-size: 18px !important; }
button[kind="primary"] { font-size: 18px !important; padding: 12px 18px !important; }
h1 { font-size: 32px !important; }
h2 { font-size: 26px !important; }
h3 { font-size: 20px !important; }
div[data-testid="stSidebar"] button[kind="primary"] { background-color: #1a73e8 !important; color: white !important; border: 2px solid #1557b0 !important; font-weight: bold !important; font-size: 18px !important; }
div[data-testid="stSidebar"] button[kind="secondary"] { background-color: transparent !important; color: #555 !important; font-size: 17px !important; }
</style>
...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        st.button("📝 项目工作台", use_container_width=True,
                  type="primary" if cur == "workspace" else "secondary",
                  on_click=_nav_to, args=("workspace",))
        st.button("📄 生成文档", use_container_width=True,
                  type="primary" if cur == "generate" else "secondary",
                  on_click=_nav_to, args=("generate",))
        ...
        if user['role'] in ('finance', 'admin'):
            ...
            st.button(al, use_container_width=True,
                      type="primary" if cur == "approval" else "secondary",
                      on_click=_nav_to, args=("approval",))
            st.button("🧾 开收据", use_container_width=True,
                      type="primary" if cur == "receipt" else "secondary",
                      on_click=_nav_to, args=("receipt",))

        if user['role'] == 'admin':
            pu = len(get_pending_users())
            al2 = f"🔒 用户审核" + (f" ({pu})" if pu else "")
            st.button(al2, use_container_width=True,
